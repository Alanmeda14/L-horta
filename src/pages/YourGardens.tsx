...
export const YourGardens = () => {
  ...
  const gardenId = 1; // ⚠️ Reemplazalo con ID dinámico si lo obtenés de la URL
  const userId = 5;   // ⚠️ Temporal: reemplazá con el ID del usuario logueado
  ...
  useEffect(() => {
    if (activeTab !== 'voluntariado') return;

    const fetchSessions = async () => {
      try {
        const data = await getSessionsByGardenId(gardenId);
        setSessions(data);
      } catch (err) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="mb-6">
          <p className="text-gray-600">Servicios disponibles: Entrega, Recogida, Visitas</p>
        </div>
        ...
        <div className="flex gap-4 mb-6 border-b">
          ...
          <button
            ...
            onClick={() => {
              ...
              setShowCreateSession(false); // Asegura que no se abra el modal automáticamente
            }}
          >
            Voluntariado
          </button>
            ...
            <div className="space-y-4">
              {sessions.map((session) => (
                <div key={session.id} className="border rounded-lg p-4">
                  ...
                  <InscriptionButton sessionId={session.id!} userId={userId} />
                </div>
              ))}
