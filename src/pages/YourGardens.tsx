...
const YourGardens: React.FC = () => {
  ...
  if (loading) return <GardensSkeleton />;
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pt-20 px-4 flex items-center justify-center">
        <div className="bg-white rounded-xl shadow-md p-8 text-center max-w-md mx-auto dark:bg-gray-800">
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pt-20 px-4 pb-16">
        ...
        {gardens.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {gardens.map((garden, index) => (
              <GardenCard key={garden.id} garden={garden} index={index} onClick={handleGardenClick} />
            ))}
          </div>
        )}
      </div>
...
const EmptyState: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <h2 className="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-4">{t("no_gardens_yet")}</h2>
      <p className="text-gray-500 dark:text-gray-400 mb-8">{t("create_your_first_garden")}</p>
      <Link
        to="/garden/new"
...
const GardenCard: React.FC<GardenCardProps> = ({ garden, index, onClick }) => {
        ...
        {garden.gardenProducts && garden.gardenProducts.length > 0 && (
          <div className="mt-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-2 flex items-center">
              <Leaf size={16} className="mr-1 text-gray-400" />
            ...
            <div className="flex flex-wrap gap-1.5">
              {garden.gardenProducts.slice(0, 3).map((product, index) => (
                <span
                  key={index}
                  className="bg-gray-100 text-gray-800 text-xs px-2 py-1 rounded-full border border-gray-200"
                >
                  {product.caName}
                </span>
              ))}
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pt-20 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div className="h-8 w-40 bg-gray-200 dark:bg-gray-700 rounded-md animate-pulse"></div>
