...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchGardens = async () => {
      try {
        const response = await getAllGardens();
        setGardens(response.content); // ✅ Aquí extraes el array
      } catch (err) {
      ...
      <div className="bg-green-700 text-white py-12 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-lg text-green-100 mb-2">
            Connecta amb la comunitat, troba productes frescos i uneix-te al
            moviment per una vida sostenible.
          </p>
        </div>
      </div>
                ...
                <div className="mt-2 flex gap-2">
                  <span
                    className={`px-2 py-1 text-xs rounded-md ${
                      garden.volunteerSessionAvailable
