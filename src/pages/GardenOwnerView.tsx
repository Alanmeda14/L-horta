          ...
          <div className="flex gap-4 mb-6 border-b border-gray-300 dark:border-gray-700">
            <button
              className={`px-4 py-2 font-medium transition-colors ${
                activeTab === 'productos'
                  ? 'text-gray-400 border-b-2 border-gray-400 dark:text-gray-400'
                  : 'text-gray-600 hover:text-gray-400 dark:text-gray-400 dark:hover:text-gray-300'
              }`}
          ...
          {activeTab === 'productos' && garden.products && (
            <div className="space-y-4">
              {garden.products.map((product: any) => (
                <div
                  key={product.id}
                  className="border p-4 rounded-lg flex justify-between items-center dark:border-gray-600"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="dark:text-gray-300">
                        {product.unitPrice} €/kg - {product.quantity}g
                      </p>
                    </div>
                  </div>
          ...
          {activeTab === 'voluntariado' && (
            <div className="space-y-4">
              {garden.sessions?.map((session: any) => (
                ...
                >
                  <h3 className="font-semibold dark:text-gray-200">{session.taskDescription}</h3>
                  <p className="dark:text-gray-300">
                    {session.date} | {session.startTime} - {session.endTime}
                  </p>
                  <p className="dark:text-gray-300">
                    Voluntarios necesarios: {session.requiredVolunteers}
                  </p>
                </div>
              ))}
